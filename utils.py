...
def quaternion_to_rotation_matrix(qvec):
    # 将四元数转换为旋转矩阵
    ### 注意：当前外参是按照xyzw排序，不是传统colmap的wxyz
    x, y, z, w = qvec
    return np.array([
...
def read_traj_file_colmap(filepath):
    ...
    with open(filepath, 'r') as file:
        for line in file:
            ...
            rotation_matrix = quaternion_to_rotation_matrix(qvec) # way1
            # rotation_matrix = Rotation.from_quat(qvec).as_matrix() # way2
...
### function：scannet坐标系→世界坐标系
def inverse_transform_coordinates(points):
    """
    将点云从 Z-up (X前, Y左) 转换回 Y-up (X前, Z右)
    转换规则：
        original_x = new_x (X保持不变)
        original_y = new_z (Z方向变为Y方向)
        original_z = -new_y (-Y方向变为Z方向)
    """
    transformed_points = np.zeros_like(points)
    transformed_points[:, 0] = points[:, 0]  # X -> X (forward)
    transformed_points[:, 1] = points[:, 2]  # Z -> Y (up to up)
    transformed_points[:, 2] = -points[:, 1]  # -Y -> Z (left to right)
    return transformed_points
...
def merge_pointclouds(semantic_pointcloud_ply_folder, transforms, is_icp, is_colmap_extrinsic):
    ...
    if is_icp:
        ...
        for pointcloud, transform in tqdm(zip(pointclouds, transforms), total=len(pointclouds)):
            ...
            if colors.max() > 1.0: # 如果颜色值可能在 0-255 范围，则归一化
                colors = colors / 255.0
            # [x, y, z] -> [x, -y, -z]
            ...
            if is_colmap_extrinsic:
                ...
                global_pointcloud.points = o3d.utility.Vector3dVector(np.vstack([np.asarray(global_pointcloud.points), transformed_points_global]))
                global_pointcloud.colors = o3d.utility.Vector3dVector(np.vstack([np.asarray(global_pointcloud.colors), colors]))
                
            else:
                pointcloud1 = pointcloud.transform(np.linalg.inv(transform))

            # pointcloud1 = transform_point_cloud(pointcloud1)
            # pointcloud_new = pointcloud_new.voxel_down_sample(voxel_size=0.08)
            # global_pointcloud += pointcloud_new       
